...
| T48 | rack-jwt-verifier: accept an in-process / callable JWKS or key-set source. TASK-020 confirmed the gap: `public_key` takes exactly one key (no rotation window) and `jwks_url` always fetches over HTTP; the hub meanwhile passes its JWKS as the ruby-jwt `jwks` decode option next to a placeholder `public_key` (config/initializers/rack_jwt_verifier.rb) — an undocumented pass-through to replace with a supported option | feat | medium | [gem: rack-jwt-verifier] | T17 | §3.2 |
| T52 | rack-jwt-verifier: RFC 9068 `typ` check (`require_typ: "at+jwt"`). Found in TASK-020: an id_token has the hub's `iss` and key, and for a client whose access tokens carry `aud` = client_id (no `resource`) also the expected `aud`, so a downstream service configured that way accepts an id_token as a bearer; only the `typ` header tells them apart | fix | high | [gem: rack-jwt-verifier] | T17 | §3.2 |
